...
# "услови\w*"/"стоимост\w*" — многие объявления пишут цену под заголовком
# "Условия:"/"Стоимость:" вместо "Цена"/"Оплата" (например "💳 Условия:
# 55.000₽ в месяц" или "Стоимость 45к")
_PRICE_KEYWORD = (
    r"(?:о)?плат(?:а|у|е|ы|ой)\b|цен(?:а|е|у|ы|ой)\b|услови\w*\b|стоимост\w*\b"
)
# запасной вариант: число рядом со словами "плата/цена/стоимость/условия"
# даже без явной валютной пометки — многие объявления специально её
# опускают или коверкают ("55.000ку"), чтобы обойти простые фильтры
PRICE_CONTEXT_RE = re.compile(
    r"(?:арендн\w*\s*плат\w*|аренда\w*|" + _PRICE_KEYWORD + r")\D{0,10}(" + _PRICE_NUM + r")",
    re.IGNORECASE,
)
...
# Необязательное "р"/"р." перед "/мес" — сокращение "рублей/месяц" без точки
# после "р" ("40.000 р/мес") не ловит PRICE_RE, там после "р" всегда нужна
# точка, а не "/"
PRICE_MONTH_RE = re.compile(
    r"(" + _PRICE_NUM + r")\s?(?:р\.?\s?)?/?\s?(?:мес\.?\b|месяц\w*)", re.IGNORECASE
)
...
# "спальное место"/"койко-место" ("койко место" тоже пишут без дефиса) —
# тоже сдача комнаты (часто в формате подселения), просто без слова
# "комната" в тексте. Но число ПЕРЕД "спальных мест" ("4 спальных места:
# кровать и диван") — это описание вместимости обычной квартиры (сколько
# человек может лечь), а не признак того, что сдаётся именно койко-место —
# такое не считаем (без этого исключения нормальная 1-к квартира
# определялась как "Комната" только из-за этой фразы про мебель)
BED_SPACE_RE = re.compile(
    r"(?<!\d)(?<!\d\s)спальн\w*\s*мест\w*|койко[\s\-]?мест\w*",
    re.IGNORECASE,
)
...
# "мебель" — используются только для того, чтобы отличить объявление, где
# ОТСУТСТВИЕ мебели указано явно ("без мебели"), от обычного случая.
# "мебел\w*" (без буквы "ь") — "мебель\w*" не матчил бы "мебели" (родительный
# падеж, как раз в "без мебели"): там после "мебел" сразу "и", без мягкого
# знака, а не "мебель"+"и"
BASIC_FURNITURE_RE = re.compile(
    r"\bмебел\w*\b|\bкроват\w*\b|\bдиван\w*\b|\bкомод\w*\b"
...
def extract_amenities(text):
    ...
    # Мебель показываем ВСЕГДА, даже если текст её не упоминает явно — почти
    # все комнаты/квартиры в таких объявлениях меблированы, это видно на
    # фото, даже когда текст об этом молчит. "без мебели" — только когда
    # это написано явно и однозначно (все упоминания мебели в тексте — с
    # отрицанием, см. _has_negated_only)
    lower = text.lower()
    found = []
    seen = set()
    if _has_negated_only(text, BASIC_FURNITURE_RE):
        found.append("без мебели")
        seen.add("без мебели")
    else:
        found.append("мебель")
        seen.add("мебель")
...
def extract_infrastructure(text):
    ...
    # названная сеть и общее упоминание "магазины" — про разные вещи и оба
    # могут быть в одном объявлении. Без слова "рядом" в самом пункте — оно
    # теперь и так идёт заголовком всего блока "Инфраструктура"
    has_standalone_generic_mention = any(
        not _span_overlaps_any(m.span(), shop_name_spans)
        for m in GENERIC_SHOP_RE.finditer(text)
    )
    if has_standalone_generic_mention and "магазины" not in seen:
        found.append("магазины")
        seen.add("магазины")
...
def extract_metro_station(text):
    ...
    if station_match:
        ...
        # узкий сырой захват мог сам обрезаться на переносе строки внутри
        # многословного названия ("м. Бульвар \nДмитрия Донского" — {0,2}
        # повторений в METRO_STATION_RE не перепрыгивают через "пробел+
        # перенос строки" как единый разделитель) — тогда справочник по
        # одному "Бульвар" ничего не найдёт. Если так, пробуем более
        # широкое окно текста сразу после "метро"/"м." — там разделитель
        # между словами в справочнике гибкий ([\s\-]+) и перенос строки
        # не помешает
        known = find_known_station(raw) or find_known_station(
            text[station_match.start(1):station_match.start(1) + 60]
        )
        return known or raw
...
# "ищу квартиру москва" — подпись фирменной ссылки/футера в конце ПОЧТИ
# КАЖДОГО поста некоторых каналов (например Find_flat) — ведёт в отдельный
# бот поиска, это не значит, что автор ЭТОГО поста сам ищет жильё. Без её
# вырезания перед проверкой keywords_exclude любое такое объявление
# ложно отсеивалось фильтром "ищу квартир" (реально отсеивающим посты
# арендаторов, а не арендодателей) — реальный случай: t.me/Find_flat/41923
BOILERPLATE_RE = re.compile(
    r"на карте"
    r"|больш\w*\s+гайд\w*\s+по\s+аренде"
    r"|больше\s+квартир\s+и\s+комнат"
    r"|ищу\s+квартиру\s+москв\w*",
    re.IGNORECASE,
)
...
# для "рекламной" перепечатки одного и того же лота сразу несколькими
# каналами/блогерами (у каждого свой маркетинговый текст, единого контакта
# в видимом тексте вообще нет — ссылка на бронирование вида "ColifeBot" не
# распознаётся как контакт) совпадение по словам ниже основного порога
# (0.6), но метро и цена идентичны — этого достаточно в связке с хотя бы
# небольшим совпадением по словам, чтобы не путать с ДРУГИМ реальным лотом
# у той же станции по случайно той же цене. Реальный случай: одна и та же
# студия у "Текстильщики" за 59 000₽, переписанная тремя разными
# блогерами — t.me/flatsforfriends/20691, t.me/lvngrm_msk/15598,
# t.me/olya_gde_zhit/4810 (общих слов 38-43%, ниже 60%)
DUPLICATE_WEAK_SIMILARITY_THRESHOLD = 0.3


def is_duplicate(words, seen_word_lists, threshold=0.6, contacts=None, price=None, metro=None):
    # seen_word_lists — либо старые записи (просто список слов, как раньше),
    # либо новые словари {"words", "contacts", "price", "metro"} (см. места,
    # где список пополняется, например fetch_new_posts_raw) — старый формат
    ...
    for entry in seen_word_lists:
        ...
        sim = similarity(words, entry_words)
        if sim >= threshold:
            return True
        ...
        # идентичны)
        if contacts and price is not None:
            entry_contacts = set(entry.get("contacts") or [])
            if entry_contacts & contacts and entry.get("price") == price:
                return True
        # третий сигнал (см. комментарий выше DUPLICATE_WEAK_SIMILARITY_THRESHOLD)
        if metro and price is not None and entry.get("metro") == metro and entry.get("price") == price:
            if sim >= DUPLICATE_WEAK_SIMILARITY_THRESHOLD:
                return True
    return False


# "ищу"/"ищет"/"ищем"/"ищут"/"ищущие" рядом (в пределах ~12 символов, без
# знаков конца предложения между ними) с типом ЖИЛЬЯ — это сам арендатор
# описывает, что ищет, а не объявление о сдаче (пример: "Ищу студию или
# 1-комнатную квартиру..." — t.me/ugnest_msk/23036). Короткое окно и явный
# список жилых существительных — чтобы НЕ задеть "Ищу арендатора на
# квартиру" (это, наоборот, нормальное объявление от собственника): там до
# "квартир" от "ищу" дальше, чем 12 символов, из-за слова "арендатора"
# между ними. Раньше это ловилось только через ручные фразы в
# keywords_exclude ("ищу квартир" и т.п.), которые не покрывают все
# формулировки ("ищу студию", "ищу жильё", "ищущие: ...")
TENANT_SEEKING_RE = re.compile(
    r"\b(?:ищ(?:у|ет|ем|ут)|ищущ\w*)\b[^.!?\n]{0,12}?"
    r"(?:студи\w*|квартир\w*|комнат\w*|жиль[её]\w*|однушк\w*|двушк\w*|тр[её]шк\w*|гнездышк\w*)",
    re.IGNORECASE,
)
...
def matches_filters(text, filters, has_photos=False):
    ...
    # ключевых слов — иначе такая подпись может ложно сработать и как
    # "включение", и как "исключение", хотя автор поста её не писал
...
# первая буква поста-продолжения (без ведущих эмодзи/пунктуации) написана
# со строчной — значит текст продолжает оборванное предложение из
# предыдущего поста ("...Заранее можно" + новый пост "провести осмотр."),
# а не начинает новое объявление (те обычно начинаются с заглавной буквы,
# эмодзи или хэштега). Запасной сигнал для _looks_like_price_continuation,
# когда в продолжении нет ни цены, ни фото — она уже была названа в первом
# посте (реальный случай: t.me/Find_flat/45074 после t.me/Find_flat/45064)
_LEADING_NON_LETTER_RE = re.compile(r"^[^a-zA-Zа-яёА-ЯЁ]+")
...
def _looks_like_price_continuation(post):
    ...
    if extract_rooms(text_no_hashtags) is not None:
        return False
    # ЦЕНА в продолжении — самый надёжный сигнал ("нет фото + нет типа
    # жилья + есть номер телефона" слишком слабый сам по себе и раньше мог
    # склеить два совсем не связанных поста подряд, например "Продам
    # диван..." и следующий "Ремонт свежий, звоните 89161234567" — у него
    # просто оказался номер в тексте, но начинается с заглавной "Ремонт",
    # это НЕ обрыв предложения). Если цены нет — запасной сигнал: начало со
    # строчной буквы (см. _starts_mid_sentence) — она тоже не задевает
    # "Ремонт свежий..." (заглавная "Р"), но ловит случаи без цены вообще
    return extract_price(post["text"]) is not None or _starts_mid_sentence(post["text"])
...
def send_recent_matching_ads(hours=48, channels=None, filters=None, chat_id=None):
    ...
    for channel in channels:
        ...
        for post in posts:
            ...
            metro_val = extract_metro_station(post["text"])
            if is_duplicate(words, seen_word_lists, contacts=contacts, price=price_val, metro=metro_val):
                continue
            seen_word_lists.append({"words": list(words), "contacts": list(contacts), "price": price_val, "metro": metro_val})
            matches.append(post)
...
def fetch_new_posts_raw(channels):
    ...
    for channel in channels:
        ...
        for post in posts:
            ...
            price_val = extract_price(post["text"])
            metro_val = extract_metro_station(post["text"])
            if is_duplicate(words, seen_word_lists + new_word_lists, contacts=contacts, price=price_val, metro=metro_val):
                # похожее объявление уже присылали из другого канала
                continue

            new_word_lists.append({"words": list(words), "contacts": list(contacts), "price": price_val, "metro": metro_val})
            results.append(post)
    ...
    new_word_lists = [
        {
            ...
            "price": extract_price(p["text"]),
            "metro": extract_metro_station(p["text"]),
        }
        for p in posts
    ]
...
def _run_scan_for_all_users():
    ...
    for chat_id in _all_registered_chat_ids():
        ...
        for p in raw_posts:
            ...
            metro_val = extract_metro_station(p["text"])
            if is_duplicate(words, seen_word_lists, contacts=contacts, price=price_val, metro=metro_val):
                continue
            seen_word_lists.append({"words": list(words), "contacts": list(contacts), "price": price_val, "metro": metro_val})
            matches.append(p)
